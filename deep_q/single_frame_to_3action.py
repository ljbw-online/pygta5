...
def create_q_model(optimizer=None, loss_func=None):
    ...
    if optimizer is not None and loss_func is not None:
        return ks.Model(inputs=inputs, outputs=q_values, optimizer=optimizer, loss=loss_func)
    else:
        return ks.Model(inputs=inputs, outputs=[q_values, layer1])
...
class Env:
    ...
    def reset(self):
        ...
        return np.expand_dims(get_frame(), axis=0)
    ...
    def step(self, action, keys):
        ...
        return np.expand_dims(get_frame(), axis=0), reward, terminated
