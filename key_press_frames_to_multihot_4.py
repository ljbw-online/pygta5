...
class InputCollector:
    ...
    def __init__(self):
        ...
        self.current_key_states = [False, False, False, False]
    ...
    # def set_current_key_states(self, keys):
    #     self.previous_key_states = self.current_key_states
    #
    #     self.current_key_states[0] = FORWARD in keys
    #     self.current_key_states[1] = LEFT in keys
    #     self.current_key_states[2] = BRAKE in keys
    #     self.current_key_states[3] = RIGHT in keys
    ...
    def set_current_key_scores(self, keys):
        ...
        for i in range(4):
            ...
            if KEYS[i] in keys and not self.current_key_states[i]:  # Key Released
                self.current_key_states[i] = True
                self.key_press_times[i] = self.t
                self.key_score_mins[i] = self.current_key_scores[i]

                # Release W when we press S
                if i == 2:
                    self.current_key_states[0] = False
            ...
            elif KEYS[i] not in keys and self.current_key_states[i]:  # Key Released
                self.current_key_states[i] = False
    ...
    def collect_input(self, keys):
        ...
        # self.set_current_key_states(keys)
        self.set_current_label(keys)
        # current_key_state already set in set_current_key_scores
...
class DataCollector(InputCollector):
    ...
    def __init__(self, array_length):
        ...
        # DATA_TARGET = array_length
        # MAX_DATA_PER_SIGNAL = array_length / NUM_SIGNALS
        ...
        self.key_states = np.zeros((array_length, 4), dtype='float32')
        ...
        # Maybe not necessary but simplifies things for now
        self.prev_key_states = [False, False, False, False]

        self.correcting = False

    def conditionally_add_datum(self):
        forward = self.current_label[0]
        left = self.current_label[1]
        brake = self.current_label[2]
        right = self.current_label[3]
        ...
        # This is no longer conditional given that every frame is recorded in this model format

        self.images[self.index] = self.current_frame
        self.key_scores[self.index] = self.current_key_scores
        self.key_states[self.index] = self.current_key_states
        self.labels[self.index] = self.current_label

        self.index += 1

        for i in range(4):
            if not self.prev_key_states[i] and self.current_key_states[i]:  # Key Pressed
                self.signal_counts[i * 4] += 1
            elif not self.prev_key_states[i] and not self.current_key_states[i]:  # Key Remaining Unpressed
                self.signal_counts[i * 4 + 1] += 1
            elif self.prev_key_states[i] and not self.current_key_states[i]:  # Key Released
                self.signal_counts[i * 4 + 2] += 1
            elif self.prev_key_states[i] and self.current_key_states[i]:  # Key Remaining Pressed
                self.signal_counts[i * 4 + 3] += 1

        self.prev_key_states = self.current_key_states.copy()

        if (self.index % round(self.array_length/100) == 0) and (self.index != 0):
            print(self.signal_counts)
    ...
    def display_frame(self):
        ...
        wasd = np.array(self.current_key_states).astype('uint8')
        wasd = wasd * 255
    ...
    def save(self):
        ...
        images_fn = 'images_uint8.npy'
        key_scores_fn = 'key_scores_float32.npy'
        key_states_fn = 'key_states_float32.npy'
        labels_fn = 'labels_float32.npy'
        ...
        self.key_states = self.key_states[:self.index]
        np.save(key_states_fn, self.key_states)
...
def train_new_model(epochs=1, model_desc=''):
    ...
    x = ks.layers.Conv2D(4, 4, padding='same', activation='relu', data_format='channels_last')(x)
    ...
    y = ks.layers.Dense(16, activation='relu')(scores_input)
